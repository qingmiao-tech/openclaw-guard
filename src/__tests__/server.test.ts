...
describe('startServer - UI selector routing', () => {
  ...
  it('serves next workbench by default on /', async () => {
  ...
  it('serves legacy workbench on / when overridden by query', async () => {
    const server = await startEphemeralServer();
    const port = getPort(server);

    const response = await fetch(`http://localhost:${port}/?ui=legacy`);
    const html = await response.text();
  ...
  it('serves legacy workbench on / when GUARD_UI=legacy', async () => {
  ...
  it('serves next workbench by default on /workbench', async () => {
    const server = await startEphemeralServer();
    const port = getPort(server);

    const response = await fetch(`http://localhost:${port}/workbench`);
    const html = await response.text();
