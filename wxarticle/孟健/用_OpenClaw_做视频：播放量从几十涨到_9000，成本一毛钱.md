![用 OpenClaw 做视频：播放量从几十涨到 9000，成本一毛钱](https://mmbiz.qpic.cn/sz_mmbiz_jpg/IzH5BgecYStdibZh4mcJMCDLU7RHCu57UAWuPIAnnXXn76OtdToj8aN3mut47cNNThbIoibxRvOOshNRvlesM1J2huDQ6AflOGyst7uYAcjlw/0?wx_fmt=jpeg)

# 用 OpenClaw 做视频：播放量从几十涨到 9000，成本一毛钱

> **作者**: 孟健的AI编程认知
> **发布日期**: 2026年2月14日 21:06
> **来源**: [原文链接](https://mp.weixin.qq.com/s/DU0IoLI8DqL6dsV91H7WBA)
...
![文章图片](https://mmbiz.qpic.cn/mmbiz_jpg/IzH5BgecYSthv0oukLrLia6H91fSVANHkfckxtfslV62yoSky6kgNh415dcGH3rf2SMaeuGO1foXFscVDp8xTiacW2AzosYTpiaN4dBN8yu17s/640?wx_fmt=jpeg&from=appmsg&watermark=1#imgIndex=0)

![文章图片](https://mmbiz.qpic.cn/mmbiz_jpg/IzH5BgecYSt9Im7gebz4o2m2jgRLoEK7SHygdarE7iaV5XENLasPehLn8h5lUpZLrJavMdm2iamFzwQwSluaTxtWVBRic6XC5uFqQatSNQjib38/640?wx_fmt=jpeg&from=appmsg&watermark=1#imgIndex=1)
...
![文章图片](https://mmbiz.qpic.cn/sz_mmbiz_png/IzH5BgecYSvFAg0RhHBQ3SnNpNib93henic7mdCWYYTTmiaKP2Zpiavw9T88hdibm143yCicHwXcAqiagGckMEV7rfbhlnb6ZXOnVqicvc9297r5zgo/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=2)
...
```
// scenes-data.ts — 这是唯一需要改的文件export const scenes: SceneData[] = [  {    start: 0.0,     // 开始时间（秒）    end: 3.46,      // 结束时间（秒）    type: 'title',  // 场景类型：决定动效    title: '三家巨头\n同一天',    xiaomo: 'peek',  // 小墨姿态  },  {    start: 3.46,    end: 5.90,    type: 'pain',    title: '微软说',    subtitle: 'Copilot 已经能写掉\n90% 的代码',    number: '90%',    highlight: 'Copilot',  },  // ... 更多场景];
```
...
![文章图片](https://mmbiz.qpic.cn/mmbiz_png/IzH5BgecYSvtR31DuMEgoibTpwumMXQ06mJQen1Am2dcw8BzhQUec3vtX9OA7iaqYQTxq41roN2XDSWHmTFfkPx0Ss9BTicI1rATnk9Dkk5Kb8/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=3)
...
```
墨媒推选题（cron 每日 9:30）    ↓ Telegram 推送5个选题孟健选一个    ↓ 选题确认墨笔写旁白脚本（60秒，200字左右）    ↓MiniMax TTS 生成克隆语音    ↓ 约¥0.1，3秒出结果Whisper 提取逐句时间戳    ↓ 本地运行，免费墨笔编排 scenes-data.ts    ↓ 按时间戳填场景类型+文案Remotion 渲染 MP4    ↓ h264编码，约2分钟墨笔发成片给孟健    ↓ Telegram 通知孟健确认 → 墨媒发布
```

**关键点：从"孟健选一个"到"成片发出来"，中间全自动。** 墨笔这个 Agent 收到选题后，自己写脚本、调 TTS、提时间戳、编场景、渲染视频、发通知。我只需要在 Telegram 里点一下确认。

![文章图片](https://mmbiz.qpic.cn/sz_mmbiz_png/IzH5BgecYSuh2EAsxmcxJAFiaw7qcSJ7fEztnDvDeKkDia0gmAnj3vC9viaLYPuyibpRRjnVX1Aibby1mRXKe2ls5swD3xuGJIRjU6b3ibdTiajgsw/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=4)
...
-   深色背景（[#0A0A0F](javascript:;)），不刺眼，高级感
...
![文章图片](https://mmbiz.qpic.cn/mmbiz_jpg/IzH5BgecYStOOj1DVjNu36I0KuG3hjic77zdhUnVHPDWUuQ5qOZ1zPuUllMH4fCrAfNvcewFHslZ1PDKYgTUUvIqctmiabIVZF4X1b8I1644M/640?wx_fmt=jpeg&from=appmsg&watermark=1#imgIndex=5)
...
![我的微信](https://mmbiz.qpic.cn/sz_mmbiz_png/IzH5BgecYSsJSNaW3cmVL6Fia3O5qVdQZTwJRAgo5F87Ev6AMMRicDFiapicHwE5DjOkb2edA1PbMIpGwydY6JSlUF3OBqY1ZvggGhiaPfGA3Omw/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=6)

  

📚 精选文章推荐

-   [AI 编程的临界点：当三家巨头同时宣布我们不写代码了](https://mp.weixin.qq.com/s?__biz=Mzk0ODM5NTEyNA==&mid=2247505156&idx=1&sn=b84cafbdad528ff7cb6ae8b5fcb3ea72&scene=21#wechat_redirect)
    
-   [测完 GLM-5 我沉默了：国产开源模型什么时候这么能打了？](https://mp.weixin.qq.com/s?__biz=Mzk0ODM5NTEyNA==&mid=2247505144&idx=1&sn=67115954fa053fea1c210a9f8810127b&scene=21#wechat_redirect)
    
-   [我用 OpenClaw 写文章，10 分钟发 14 个平台，全流程拆解](https://mp.weixin.qq.com/s?__biz=Mzk0ODM5NTEyNA==&mid=2247505093&idx=1&sn=d64127421cab99d94c562922039403ce&scene=21#wechat_redirect)
