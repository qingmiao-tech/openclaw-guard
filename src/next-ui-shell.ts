...
const RUNTIME_BOOTSTRAP = JSON.stringify({
  mode: 'web',
  ...
  <div id="guard-next-app">
    <div style="padding:24px;font-family:'Segoe UI','PingFang SC','Microsoft YaHei',sans-serif;color:#dceaf8;background:#08111f;min-height:100vh;">
      <div style="max-width:960px;margin:0 auto;">
        <p style="margin:0 0 12px;font-size:12px;letter-spacing:0.16em;text-transform:uppercase;color:#8bb7d9;">OpenClaw Guard</p>
        <h1 style="margin:0 0 12px;font-size:32px;">Loading Guard console…</h1>
        <p style="margin:0;line-height:1.7;color:#9cb4ca;">If this screen stays here, rebuild the web shell with <code>npm run build:web</code> in the package root.</p>
      </div>
    </div>
  </div>
  <script>window.__OPENCLAW_GUARD_RUNTIME__ = ${RUNTIME_BOOTSTRAP};</script>
