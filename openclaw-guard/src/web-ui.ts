...
`;
  ...
  const js = [
    ...
    ' zh: { dashboard: "仪表盘", info: "系统", openclaw: "OpenClaw", feishu: "飞书", channels: "渠道", ai: "AI 模型", mission: "任务", audit: "审计", profiles: "预设", harden: "加固", logs: "日志" }',
    '};',
    '',
    'var I18N_ZH_PAIRS = [',
    ' ["OpenClaw Guard - Security Dashboard","OpenClaw Guard - 安全控制台"],',
    ' ["If copy fails, ensure Node.js v22+ is installed, then run onboard.","如果复制失败，请先确认 Node.js v22+ 已安装，再执行 onboard。"],',
    ' ["Use comma-separated provider/model IDs. Order matters.","使用逗号分隔 provider/model，顺序即优先级。"],',
    ' ["Configure channel defaults and runtime settings.","配置各渠道默认值与运行参数。"],',
    ' ["Mission API Token (optional for remote calls)","Mission API Token（远程调用可选）"],',
    ' ["Keep empty for loopback-only usage","留空表示仅允许本机回环调用"],',
    ' ["Task is in progress. Waiting for status updates...","任务执行中，等待状态更新..."],',
    ' ["Mission command completed, but status confirmation timed out","Mission 命令已完成，但状态确认超时"],',
    ' ["Current status: running=","当前状态: running="],',
    ' ["Status confirmation pending: ","状态确认等待中："],',
    ' ["Current version: ","当前版本："],',
    ' ["Latest version: ","最新版本："],',
    ' ["Log auto refresh enabled (3s interval)","日志自动刷新已启用（3秒间隔）"],',
    ' ["Log auto refresh disabled","日志自动刷新已关闭"],',
    ' ["No configurable keys returned for this channel.","当前渠道没有可配置字段。"],',
    ' ["Please provide Provider name and Base URL","请填写 Provider 名称和 Base URL"],',
    ' ["Please select at least one model","请至少选择一个模型"],',
    ' ["Choose a preset to bootstrap provider configuration.","选择一个预设快速创建 Provider 配置。"],',
    ' ["Failed to load logs: ","加载日志失败："],',
    ' ["Load presets failed: ","加载预设失败："],',
    ' ["Load failed: ","加载失败："],',
    ' ["Service is not running","服务未运行"],',
    ' ["OpenClaw installed","OpenClaw 已安装"],',
    ' ["Update available","发现可用更新"],',
    ' ["Security Profiles","安全预设"],',
    ' ["Hardening Steps","加固步骤"],',
    ...
    ' ["OpenClaw Home","OpenClaw 目录"],',
    ' ["OpenClaw Dir","OpenClaw 目录"],',
    ' ["Audit Summary","审计汇总"],',
    ' ["Pass: ","通过："],',
    ' ["Warn: ","警告："],',
    ' ["Fail: ","失败："],',
    ' ["Fix: ","修复建议："],',
    ' ["Feishu Plugin Status","飞书插件状态"],',
    ' ["Basic Credentials","基础凭据"],',
    ' ["Access Control","访问控制"],',
    ...
    ' ["Webhook Host","Webhook 地址"],',
    ' ["Webhook Port","Webhook 端口"],',
    ' ["DM Policy (dmPolicy)","私聊策略 (dmPolicy)"],',
    ' ["Group Policy (groupPolicy)","群聊策略 (groupPolicy)"],',
    ' ["Require @mention","必须 @ 提及"],',
    ' ["Enable Streaming","启用流式输出"],',
    ' ["Channel Configuration","渠道配置"],',
    ' ["Primary Model","主模型"],',
    ' ["Fallback Models","回退模型"],',
    ' ["Save Fallbacks","保存回退链"],',
    ' ["Configured Providers","已配置 Provider"],',
    ' ["No providers configured yet.","尚未配置任何 Provider。"],',
    ...
    ' ["Mission Task Timeline","Mission 任务时间线"],',
    ' ["Mission Control Logs","Mission 控制日志"],',
    ' ["Task running","任务进行中"],',
    ' ["Health check OK","健康检查通过"],',
    ' ["Health check failed","健康检查失败"],',
    ' ["Install/Update","安装/更新"],',
    ' ["Start DEV","启动 DEV"],',
    ' ["Start PROD","启动 PROD"],',
    ' ["Restart DEV","重启 DEV"],',
    ' ["Restart PROD","重启 PROD"],',
    ' ["Save Config","保存配置"],',
    ' ["Delete Config","删除配置"],',
    ' ["Save Token","保存 Token"],',
    ' ["Delete provider: ","删除 Provider："],',
    ' ["Clear this channel configuration?","确认清空该渠道配置？"],',
    ' ["Loading logs...","加载日志中..."],',
    ' ["No logs yet","暂无日志"],',
    ' ["Loading OpenClaw...","加载 OpenClaw 中..."],',
    ' ["Loading...","加载中..."],',
    ' ["Starting...","启动中..."],',
    ' ["Stopping...","停止中..."],',
    ' ["Restarting...","重启中..."],',
    ...
    ' document.title = guardLang === "zh" ? "OpenClaw Guard - 安全控制台" : "OpenClaw Guard - Security Dashboard";',
    ' updateTabLabels();',
    ' updateLangButtons();',
    ' if (guardLang === "zh") {',
    ' startI18nObserver();',
  ...
  return '<!DOCTYPE html>\n'
    ...
    + '   <button class="lang-btn" id="lang-btn-zh" type="button">中文</button>\n'
