...
`;
  ...
  const js = [
    ...
    '      + \'<button class="btn success" onclick="svcAction(\\x27start\\x27)" \' + (running ? "disabled" : "") + \'>▶ 启动</button>\'',
    '      + \'<button class="btn danger" onclick="svcAction(\\x27stop\\x27)" \' + (!running ? "disabled" : "") + \'>⏹ 停止</button>\'',
    '      + \'<button class="btn warning" onclick="svcAction(\\x27restart\\x27)">🔄 重启</button>\'',
    '      + \'</div></div>\';',
    '',
    '    // Gateway Token',
